          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {blogs.map((blog) => (
              <Card key={blog.slug} className="group flex flex-col overflow-hidden border-0 shadow-md rounded-2xl hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                {/* Image placeholder */}
                <div className="h-48 bg-linear-to-br from-[hsl(152,40%,92%)] to-[hsl(152,30%,85%)] w-full flex items-center justify-center overflow-hidden">
                  <div className="text-comfindo-green opacity-15 text-4xl font-black">comfindo</div>
                </div>
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-center mb-2">
                    <Badge variant="secondary" className="bg-comfindo-green/8 text-comfindo-green border-0 text-xs">
                      {blog.category}
                    </Badge>
                    <span className="text-xs text-gray-400">{blog.date}</span>
                  </div>
                  <CardTitle className="text-lg line-clamp-2">
                    <Link href={`/blog/${blog.slug}`} className="hover:text-comfindo-green transition-colors">
                      {blog.title}
                    </Link>
                  </CardTitle>
                </CardHeader>
                ...
                <CardFooter>
                  <Button
                    asChild
                    variant="ghost"
                    className="text-comfindo-green hover:bg-comfindo-green/5 px-0 group/btn"
                  >
