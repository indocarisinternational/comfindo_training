      ...
      <div className="container max-w-4xl mx-auto px-5 sm:px-8 lg:px-16 xl:px-20">
        <div className="text-center mb-14">
          ...
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-gray-900">
            Pertanyaan <span className="text-comfindo-green">Umum</span>
          </h2>
        ...
        <Accordion type="single" collapsible className="w-full space-y-3">
          {faqs.map((faq, i) => (
            <AccordionItem
              key={i}
              value={`item-${i}`}
              className="border border-gray-100 rounded-xl px-6 data-[state=open]:border-comfindo-green/30 data-[state=open]:bg-comfindo-green/3 transition-all shadow-sm"
            >
              <AccordionTrigger className="text-left font-semibold text-gray-900 hover:text-comfindo-green hover:no-underline py-5">
                {faq.question}
              </AccordionTrigger>
